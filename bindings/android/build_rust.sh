...
aarch64-linux-android21-clang++ -shared -stdlib=libc++ swig_wrap.cxx -L$RUSTLIB/target/aarch64-linux-android/release/ -o $JNILIBS/arm64-v8a/librgb.so -fPIC 
...
x86_64-linux-android21-clang++ -shared -stdlib=libc++ swig_wrap.cxx -L$RUSTLIB/target/x86_64-linux-android/release/ -o $JNILIBS/x86_64/librgb.so -fPIC 
#cp -v $RUSTLIB/target/x86_64-linux-android/release/librgb.so $JNILIBS/x86_64/
cp -v $NDK_HOME/sources/cxx-stl/llvm-libc++/libs/x86_64/libc++_shared.so $JNILIBS/x86_64/

armv7a-linux-androideabi21-clang++ -shared -stdlib=libc++ swig_wrap.cxx -L$RUSTLIB/target/armv7-linux-androideabi/release/ -o $JNILIBS/armeabi-v7a/librgb.so -fPIC 
#cp -v $RUSTLIB/target/armv7-linux-androideabi/release/librgb.so $JNILIBS/armeabi-v7a/
cp -v $NDK_HOME/sources/cxx-stl/llvm-libc++/libs/armeabi-v7a/libc++_shared.so $JNILIBS/armeabi-v7a/

i686-linux-android21-clang++ -shared -stdlib=libc++ swig_wrap.cxx -L$RUSTLIB/target/i686-linux-android/release/ -o $JNILIBS/x86/librgb.so -fPIC 
#cp -v $RUSTLIB/target/i686-linux-android/release/librgb.so $JNILIBS/x86/
